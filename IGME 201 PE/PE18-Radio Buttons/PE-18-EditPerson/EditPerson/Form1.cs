    ...
    public partial class PersonEditForm : Form
    {
            ...
            */
            ...
            if (person.GetType() == typeof(Student))
            {
                ...
                if (person.name == "") //when we initially have a person we are adding and not editing they
                                       //dont have a name (defulat name was not set) so we want a default year since noththing will be selected right??****(1)
                {
                    student.eCollegeYear = collegeYear.senior; //we can't use person reference here only a student since student only uses college year right??****(2)
                }
                ...
                switch (student.eCollegeYear) //we set the college year in the ok button click method so now if we want to edit the same person
                                              ...
                                              //to have their saved data loaded for them right??(3)**
                {
                    case collegeYear.freshman:
                        ...
                        //when we first enter the form we have to comapre what was stored with the enumerated type values to display the data and in the ok button is where
                        //we edit it the persons year and gender??(4)*****
                        break;
            ...
            if (person.name == "")
            {
                person.eGender = genderPronoun.them; //is this how we access because it's used in teacher
                                                     //and student so we have to use the person reference??(5)*****
            }
        ...
        private void ClassRadioButton__CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (rb.Checked)
            {
                ...
                {
                    this.classLabel.Text = "Class of 2023"; 
                }
        ...
        private void TypeComboBox__SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if(cb.SelectedIndex == 0 )
            {
                ...
                this.classGroupBox.Visible = true; //this sets the groupbox and radio buttons to
                                                   //be shown if its a studentright?? and everything in a groupbox will show when we set
                                                   //the groupbox itself to visible??(6)*****
                ...
                if (!this.freshmanRadioButton.Checked && !this.sophomoreRadioButton.Checked && !this.juniorRadioButton.Checked && !this.seniorRadioButton.Checked)
                {
                    ...
                    //to student again would the student freshman radio button we selected still be selected or no??(8)****
                }
            }
            else
            {
                ...
                this.classGroupBox.Visible = false; //if its a teacher we dont want it to show to classgroupbox right??****(9)
            }
        }
        ...
        private void OkButton__Click(object sender, EventArgs e)
        {
            ...
            if (this.himRadioButton.Checked)
            {
                ...
                //when we press ok so it gets saved when we save the person as well as when we are editing the person it selects
                //what we have currently selected but this is only as we are editing the person??(10)*****
            }
            ...
            if ( person.GetType() == typeof( Student))
            {
                ...
                if (this.seniorRadioButton.Checked) //can we also do else if instead of doing if over and over again??(11)****
                {
