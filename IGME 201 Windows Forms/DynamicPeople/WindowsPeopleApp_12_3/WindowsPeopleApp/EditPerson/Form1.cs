...
namespace EditPerson
{
    ...
    {
        public Person formPerson; //allows dynamic people application to access the formPerson but if they change the email adddress then we can no longer
        //access person through the sortedList because we dont know what the new email address they put in is (cant get updated information for person
        //to show in the panel so we need to do something to support that******
        ...
        private void OkButton__Click(object sender, EventArgs e)
        {
            ...
            //set this to person so that the updated data will be available outside of the form******
            formPerson = person;

            // hide the form instead of closing it so that the form persists in memory
            //have this instead of close and dispose so then the form object still exists when we are in another window and we can still access the data within another
            //form*****
            this.Hide();
            ...
            //if we did close and dispose it would close the main form and only the one we selected on that specific person would be open??*****
            //this.Close();
            //this.Dispose();
        }
