...
namespace BTree
{
    ...
    public class BTree
    {
        ...
        //////////////////////////////////////////////////////////
        /// The following 2 traversal methods can be used to duplicate the tree
        //////////////////////////////////////////////////////////
        public static void DepthFirstPreOrderTraversal(BTree node)
        {
            //ASK:
            //we first take in a node then if the node we passed in is not null (base case for recursion)**
            //and if the node is not a seed node (a primed node) then we print the node
            //then we call the method recursively for the left and right child 
            //and it basically goes down the left children first then goes back up to the root node
            //then goes down the right side all the way down**

            //how does it know to stop using the recursion for the left side of the root and start doing the right side
            //and to not implement the recursion for the left side over and over again** (how does it get to the recursion of 
            //the right side if it's below the recursion of the left side)**

            //why do we want to print it out if its data but not if its a seed or primed node for the tree**

            //for the node we pass in is it usually the root node or could we also pass in a node from the middle of the tree
            //and just go down from there for the left and right children of that specific node**
            if (node != null)
            {
                if (node.isData)
                {
                ...
                DepthFirstPreOrderTraversal(node.ltChild);
                DepthFirstPreOrderTraversal(node.gteChild);
            }
        }


        //ASK:
        //in terms of calling this method in the exam would we use the enumerated type number or would the enumerated type 
        //value be ok to use since the enumerated type is class scoped**
        //how would it know to got through the values in the exam question because here it's a tree so how would it be different**
        //its also weighted so I was just confused on if we had to account for that or if we could just read vertically down from the image
        //if we rotated it**
        //go over exam question**

        public static void BreadthFirstTopDownTraversal(BTree root)
        {
            //ASK:

            //queue to hold our root node at first 
            Queue<BTree> queue = new Queue<BTree>();
            queue.Enqueue(root);
            while (queue.Count != 0)
            {
                //dequeue the root node at first and get the first level of the left and right node**
                //then when the second iteration happens it takes out the left child after the root (oldest data) then gets their left and right child
                //and prints out the data if its not a seed node**
                //for the next iteration it then does the right node after the root (now the oldest data) then prints out
                //the data if its not a seed node then gets the specific left and right child for that node**
                //each iteration keeps going down the levels of the tree until there is nothing in the queue to dequeue (the count is 0)**
                BTree node = queue.Dequeue();
                if (node.isData)
                {
                    //if the node is not a seed or a primed node then print it out**
                    //why do we want to print it out if its not seed data**
                    PrintThisNode(node);
                }
