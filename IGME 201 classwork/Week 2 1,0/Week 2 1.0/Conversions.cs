...
namespace Week_2_1._0
{
    static internal partial class Program
    {
        static void Main(string[] args)
        {
            ...
            try //need this try catch or the program will crash and if we convert to more to less percise then it would crash if the data type would not fit**
                ...
                //run time error without the try catch**
            {
                ...
                byteNum = byte.Parse(shortInt.ToString()); //we would need to convert our things into. strings to use parse if they are not strings**
            ...
            //int/int = int (even though we stores the int into a double
            doubleNum = longInt / shortInt; //double num was a double but since we divided whole int. then the answer is an int.**
            //we cant initizalize and int. to a double, we can only initiazlize and int. to an int.**
            ...
            //press the project not the solution the right click, then add a new class 
            //rename files by right clocking and rename then press no 
            ...
            //and made sure to put static in front of the class and added partial before the keyword class in both files** 
        }
    }
}
