...
namespace Week_5_1._0
{
    ...
    static internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //when we constrcut the data type on the rght hand side of = side, then it creates the constrcture with the new keyword** and the ()**
            ...
            dPersonAge.Add("sue", 84); //dictionaries can store a mix of datatypes as well as structs and enums and classes and datatype (same for lists)**
            ...
            foreach (KeyValuePair<string, int> valuePair in dPersonAge) //goes through all the keys and values in the dict.**
                //always use the keyvalue pair and name is then go through the dict name**
            {
            ...
            dPersonAge.Add("sue", 84); //this is how we created a sorted list as we put the values in so it gets stored or the way below it also works**
            dPersonAge["david"] = 50; 
            personAge["joe"] = 80;
            ...
            //how do we know to access something by key or value or is it always the key**
            //values can repeat in a dict.
            personAge.Remove("joe");

            //the dict. lets us read the list(dict). using a name and we can access it right by saying the key and indexing rather than if it was a list we would
        ...
        static int Average(int nRoundTo, params int[] aInt) //params. lets us pass a list to the method and put it at the end because its inifinte amount of values it can hold
        {
            ...
            avg = Math.Round((double) (sum) / (oubleaInt.Length, nRoundTo);
            return avg;
        }
        ...
        //
        //Factorial(2) = 1 * 2 = 2 //when the method calls itself it launces a new instance of itself and it spawns a new instance of the method running 
        ...
        //in c# we can define classes as being publuc or interncal or public/internal abstract,sealed,or static (protected is also used in classes)**

        //class box has the type of class then below is the fields then methods and prop. then constrcutors (any data type that is not a property is a field**
        
        //inheritence only has 1 inheritence (one parent only)**
    }
}
